    ...
    public class CameraZoom : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (currentSpeed > 0.1f) // If player is moving
                virtualCamera.m_Lens.OrthographicSize = Mathf.Lerp(virtualCamera.m_Lens.OrthographicSize, normalSize,
                    Time.deltaTime * zoomSpeed);
            else // If player is not moving
                virtualCamera.m_Lens.OrthographicSize = Mathf.Lerp(virtualCamera.m_Lens.OrthographicSize, zoomedInSize,
                    Time.deltaTime * zoomSpeed);
        }
    }
}