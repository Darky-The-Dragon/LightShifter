        ...
        public override void OnInspectorGUI()
        {
            ...
            switch (currentTab)
            {
                case 0:
                    ...
                    if (mwTarget.showConfirmButton && mwTarget.confirmButton != null &&
                        GUILayout.Button("Edit Confirm Button", customSkin.button))
                        Selection.activeObject = mwTarget.confirmButton;
                    if (mwTarget.showCancelButton && mwTarget.cancelButton != null &&
                        GUILayout.Button("Edit Cancel Button", customSkin.button))
                        Selection.activeObject = mwTarget.cancelButton;
                    GUILayout.EndHorizontal();

                    if (Application.isPlaying == false)
                    {
                        if (mwTarget.GetComponent<CanvasGroup>().alpha == 0 &&
                            GUILayout.Button("Set Visible", customSkin.button))
                        {
                            mwTarget.GetComponent<CanvasGroup>().alpha = 1;
                            EditorSceneManager.MarkSceneDirty(SceneManager.GetActiveScene());
                        }

                        else if (mwTarget.GetComponent<CanvasGroup>().alpha == 1 &&
                                 GUILayout.Button("Set Invisible", customSkin.button))
                        {
                    ...
                    if (mwTarget.useCustomContent == false && mwTarget.useLocalization)
                    {
                        ...
                        HeatUIEditorHandler.DrawProperty(descriptionKey, customSkin, "Description Key",
                ...
                case 2:
                    ...
                    closeOnCancel.boolValue = HeatUIEditorHandler.DrawToggle(closeOnCancel.boolValue, customSkin,
                        "Close Window On Cancel");
                    closeOnConfirm.boolValue = HeatUIEditorHandler.DrawToggle(closeOnConfirm.boolValue, customSkin,
                        "Close Window On Confirm");
                    showCancelButton.boolValue = HeatUIEditorHandler.DrawToggle(showCancelButton.boolValue, customSkin,
                        "Show Cancel Button");
                    showConfirmButton.boolValue = HeatUIEditorHandler.DrawToggle(showConfirmButton.boolValue,
                        customSkin, "Show Confirm Button");
