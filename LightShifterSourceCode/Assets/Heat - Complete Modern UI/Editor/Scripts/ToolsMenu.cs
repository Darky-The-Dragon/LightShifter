    ...
    public class ToolsMenu : Editor
    {
        ...
        private static void ShowErrorDialog()
        {
            EditorUtility.DisplayDialog("Heat UI", "Cannot create the object due to missing manager file. " +
                                                   "Make sure you have 'Heat UI Manager' file in Heat UI > Resources folder.",
                "Dismiss");
        }
        ...
        private static void ShowManager()
        {
            ...
            if (Selection.activeObject == null)
                Debug.Log(
                    "<b>[Heat UI]</b>Can't find an asset called 'Heat UI Manager'. Make sure you have 'Heat UI Manager' in: Heat UI > Editor > Resources");
        }
        ...
        private static void CreateCanvas()
        {
            try
            {
                ...
                UpdateCustomEditorPath();
                var clone = Instantiate(
                    AssetDatabase.LoadAssetAtPath(objectPath + "UI Elements/Canvas/Canvas" + ".prefab",
        ...
        private static void CreateButtonMain()
        {
            CreateObject("UI Elements/Button/Button");
        }

        [MenuItem("GameObject/Heat UI/Button/Button (Box)", false, 8)]
        private static void CreateButtonBox()
        {
            CreateObject("UI Elements/Button/Button (Box)");
        }

        [MenuItem("GameObject/Heat UI/Button/Button (Box Icon Only)", false, 8)]
        private static void CreateButtonBoxIO()
        {
            CreateObject("UI Elements/Button/Button (Box Icon Only)");
        }

        [MenuItem("GameObject/Heat UI/Button/Button (Panel)", false, 8)]
        private static void CreateButtonPanel()
        {
            CreateObject("UI Elements/Button/Button (Panel)");
        }

        [MenuItem("GameObject/Heat UI/Button/Button (Panel Alt)", false, 8)]
        private static void CreateButtonPanelAlt()
        {
            CreateObject("UI Elements/Button/Button (Panel Alt)");
        }

        [MenuItem("GameObject/Heat UI/Button/Button (Radial Icon Only)", false, 8)]
        private static void CreateButtonRadialIO()
        {
            CreateObject("UI Elements/Button/Button (Radial Icon Only)");
        }

        [MenuItem("GameObject/Heat UI/Button/Button (Shop)", false, 8)]
        private static void CreateButtonShop()
        {
            CreateObject("UI Elements/Button/Button (Shop)");
        }

        [MenuItem("GameObject/Heat UI/Dropdown/Standard", false, 8)]
        private static void CreateDropdown()
        {
            CreateObject("UI Elements/Dropdown/Dropdown");
        }

        [MenuItem("GameObject/Heat UI/HUD/Health Bar", false, 8)]
        private static void CreateHudHealthBar()
        {
            CreateObject("HUD/Health Bar");
        }

        [MenuItem("GameObject/Heat UI/HUD/Minimap", false, 8)]
        private static void CreateHudMinimap()
        {
            CreateObject("HUD/Minimap");
        }

        [MenuItem("GameObject/Heat UI/HUD/Quest Item", false, 8)]
        private static void CreateHudQuestItem()
        {
            CreateObject("HUD/Quest Item");
        }

        [MenuItem("GameObject/Heat UI/Input/Hotkey Indicator", false, 8)]
        private static void CreateHotkeyIndicator()
        {
            CreateObject("UI Elements/Input/Hotkey Indicator");
        }
        ...
        private static void CreateSettingsDropdownt()
        {
            CreateObject("UI Elements/Settings/Settings Element (Dropdown Alt)");
        }

        [MenuItem("GameObject/Heat UI/Settings/Settings Element (Horizontal Selector)", false, 8)]
        private static void CreateSettingsHS()
        {
            CreateObject("UI Elements/Settings/Settings Element (Horizontal Selector)");
        }

        [MenuItem("GameObject/Heat UI/Settings/Settings Element (Slider)", false, 8)]
        private static void CreateSettingsSlider()
        {
            CreateObject("UI Elements/Settings/Settings Element (Slider)");
        }

        [MenuItem("GameObject/Heat UI/Settings/Settings Element (Switch)", false, 8)]
        private static void CreateSettingsSwitch()
        {
            CreateObject("UI Elements/Settings/Settings Element (Switch)");
        }
        ...
        {
            CreateObject("UI Elements/Text/Text (TMP)");
        }

        [MenuItem("GameObject/Heat UI/Widgets/News Slider", false, 8)]
        private static void CreateNewsSlider()
        {
            CreateObject("UI Elements/Widgets/News Slider/News Slider");
        }

        [MenuItem("GameObject/Heat UI/Widgets/Socials", false, 8)]
        private static void CreateSocialsWidget()
        {
            CreateObject("UI Elements/Widgets/Socials/Socials Widget");
        }
    }
}
#endif