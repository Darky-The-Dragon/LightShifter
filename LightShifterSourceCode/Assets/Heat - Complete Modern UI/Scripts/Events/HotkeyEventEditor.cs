...
namespace Michsky.UI.Heat
{
    ...
    public class HotkeyEventEditor : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            if (!Application.isPlaying && heTarget.hotkeyType == HotkeyEvent.HotkeyType.Dynamic)
                ...
                case 0:
                    ...
                    if (heTarget.hotkeyType == HotkeyEvent.HotkeyType.Dynamic)
                    {
                        EditorGUILayout.HelpBox(
                            "Dynamic: UI will adapt itself to the current controller scheme (if available). Recommended if you want to specify the hotkey in UI.",
                            MessageType.Info);
                        HeatUIEditorHandler.DrawProperty(controllerPreset, customSkin, "Default Preset");
                        if (labelObj.objectReferenceValue != null)
                            HeatUIEditorHandler.DrawProperty(hotkeyLabel, customSkin, "Hotkey Label");

                        GUILayout.BeginHorizontal(EditorStyles.helpBox);
                        EditorGUILayout.LabelField(new GUIContent("Key ID"), customSkin.FindStyle("Text"),
                            GUILayout.Width(120));
                        EditorGUILayout.PropertyField(keyID, new GUIContent(""));
                        heTarget.showOutputOnEditor = GUILayout.Toggle(heTarget.showOutputOnEditor,
                        ...
                        if (heTarget.controllerPreset != null)
                        {
                            ...
                            EditorGUILayout.LabelField(
                                new GUIContent("Search for keys in " + heTarget.controllerPreset.name),
                            ...
                            if (!string.IsNullOrEmpty(searchString))
                            {
                                ...
                                for (var i = 0; i < heTarget.controllerPreset.items.Count; i++)
                                    if (heTarget.controllerPreset.items[i].itemID.ToLower()
                                        .Contains(searchString.ToLower()))
                                        if (GUILayout.Button(new GUIContent(heTarget.controllerPreset.items[i].itemID),
                                                customSkin.button))
                                        {
                            ...
                            if (heTarget.showOutputOnEditor)
                            {
                                GUI.enabled = false;
                                for (var i = 0; i < heTarget.controllerPreset.items.Count; i++)
                                    ...
                                    {
                                        EditorGUILayout.LabelField(
                                            new GUIContent("Output for " + heTarget.controllerPreset.items[i].itemID),
                                            customSkin.FindStyle("Text"));
                                        EditorGUILayout.LabelField(
                                            new GUIContent("[Type] " + heTarget.controllerPreset.items[i].itemType),
                                            customSkin.FindStyle("Text"));
                                        if (heTarget.controllerPreset.items[i].itemType ==
                                            ControllerPreset.ItemType.Text)
                                            EditorGUILayout.LabelField(
                                                new GUIContent("[Text] " + heTarget.controllerPreset.items[i].itemText),
                                                customSkin.FindStyle("Text"));
                                        else if (heTarget.controllerPreset.items[i].itemIcon != null)
                                            GUILayout.Box(
                                                HeatUIEditorHandler.TextureFromSprite(heTarget.controllerPreset.items[i]
                                                    .itemIcon), GUILayout.Width(32), GUILayout.Height(32));
                                    }
                        ...
                        if (Application.isPlaying == false &&
                            GUILayout.Button(new GUIContent("Update Visual UI"), customSkin.button))
                        {
                            heTarget.UpdateVisual();
                    ...
                    {
                        EditorGUILayout.HelpBox(
                            "Custom: You can manually change the content, but UI will not automatically adapt to the current controller scheme. " +
