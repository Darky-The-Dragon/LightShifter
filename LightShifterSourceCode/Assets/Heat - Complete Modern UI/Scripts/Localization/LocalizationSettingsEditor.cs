...
namespace Michsky.UI.Heat
{
    ...
    public class LocalizationSettingsEditor : Editor
    {
        ...
        private List<string> languageList = new();
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.LabelField(new GUIContent("Default Language"), customSkin.FindStyle("Text"),
                GUILayout.Width(120));
            ...
            if (languageList.Count != 0)
            {
                ...
                defaultLanguageIndex.intValue =
                    EditorGUILayout.Popup(defaultLanguageIndex.intValue, languageList.ToArray());
                defaultLanguageID.stringValue = languageList[defaultLanguageIndex.intValue];
            }
            else
            {
                EditorGUILayout.HelpBox("There are no available languages.", MessageType.Info);
            }
            ...
            // Draw languages
            for (var i = 0; i < lsTarget.languages.Count; i++)
            {
                // Draw Action Buttons
                GUILayout.Space(6);
                ...
                if (GUILayout.Button("Delete", customSkin.button, GUILayout.Width(50)))
                    if (EditorUtility.DisplayDialog("Delete Language",
                            ...
                            lsTarget.languages[i].languageName + " (" + lsTarget.languages[i].languageID + ")"
                            + "\n\nThis action deletes the specified language resources and cannot be undone.", "Yes",
                            "Cancel"))
                    {
                        if (lsTarget.languages[i].localizationLanguage != null)
                            AssetDatabase.DeleteAsset(
                                AssetDatabase.GetAssetPath(lsTarget.languages[i].localizationLanguage));
                ...
                GUILayout.Space(4);
                GUILayout.BeginHorizontal();
                if (string.IsNullOrEmpty(lsTarget.languages[i].languageName))
                    lsTarget.languages[i].isExpanded = EditorGUILayout.Foldout(lsTarget.languages[i].isExpanded,
                        "Language #" + i, true, foldoutStyle);
                else
                    lsTarget.languages[i].isExpanded = EditorGUILayout.Foldout(lsTarget.languages[i].isExpanded,
                        lsTarget.languages[i].languageName + " (" + lsTarget.languages[i].languageID + ")", true,
                        foldoutStyle);
                lsTarget.languages[i].isExpanded = GUILayout.Toggle(lsTarget.languages[i].isExpanded,
                ...
                {
                    lsTarget.languages[i].languageID =
                        EditorGUILayout.TextField("Language ID", lsTarget.languages[i].languageID);
                    lsTarget.languages[i].languageName =
                        EditorGUILayout.TextField("Language Name", lsTarget.languages[i].languageName);
                    lsTarget.languages[i].localizedName =
                        EditorGUILayout.TextField("Localized Name", lsTarget.languages[i].localizedName);
                    lsTarget.languages[i].localizationLanguage = EditorGUILayout.ObjectField("Language Asset",
                        lsTarget.languages[i].localizationLanguage, typeof(LocalizationLanguage),
                        true) as LocalizationLanguage;

                    if (lsTarget.languages[i].localizationLanguage == null &&
                        GUILayout.Button("Create Language Asset", customSkin.button))
                    {
                        var newLocale = CreateInstance<LocalizationLanguage>();

                        // Check for the path
                        var path = AssetDatabase.GetAssetPath(lsTarget);
                        path = path.Replace("/" + lsTarget.name + ".asset", "").Trim();
                        var fullPath = path.Replace("/" + lsTarget.name + ".asset", "").Trim() + "/Languages/";
                        ...
                        for (var x = 0; x < lsTarget.tables.Count; x++)
                        {
                            var newList = new LocalizationLanguage.TableList();
                            ...
                            for (var y = 0; y < lsTarget.tables[x].localizationTable.tableContent.Count; y++)
                            {
                                var newContent = new LocalizationLanguage.TableContent();
                        ...
                        AssetDatabase.RenameAsset(AssetDatabase.GetAssetPath(newLocale),
                            lsTarget.languages[i].languageName + " (" + lsTarget.languages[i].languageID + ")");
                    ...
                    else if (lsTarget.languages[i].localizationLanguage != null &&
                             GUILayout.Button("Show Language Asset", customSkin.button))
                    {
                ...
                if (lsTarget.languages[i].localizationLanguage != null)
                {
                    ...
                    lsTarget.languages[i].localizationLanguage.languageID = lsTarget.languages[i].localizationLanguage
                        .localizationSettings.languages[i].languageID;
                    lsTarget.languages[i].localizationLanguage.languageName = lsTarget.languages[i].localizationLanguage
                        .localizationSettings.languages[i].languageName;
                    lsTarget.languages[i].localizationLanguage.localizedName = lsTarget.languages[i]
                        .localizationLanguage.localizationSettings.languages[i].localizedName;
                }
            ...
            {
                var language = new LocalizationSettings.Language();
            ...
            // Draw tables
            for (var i = 0; i < lsTarget.tables.Count; i++)
            {
                // Draw Action Buttons
                GUILayout.Space(6);
                GUILayout.BeginHorizontal();
                ...
                if (lsTarget.tables[i].localizationTable != null &&
                    GUILayout.Button("Edit", customSkin.button, GUILayout.Width(34)))
                ...
                if (GUILayout.Button("Delete", customSkin.button, GUILayout.Width(50)))
                    if (EditorUtility.DisplayDialog("Delete Table",
                            "Are you sure you want to delete the following table: " + lsTarget.tables[i].tableID
                            + "\n\nThis action deletes the specified table resources and cannot be undone.", "Yes",
                            "Cancel"))
                    {
                        // Clear empty languages first
                        for (var x = 0; x < lsTarget.languages.Count; x++)
                        for (var y = 0; y < lsTarget.languages[x].localizationLanguage.tableList.Count; y++)
                        ...
                        if (lsTarget.tables[i].localizationTable != null)
                            for (var x = 0; x < lsTarget.languages.Count; x++)
                            {
                                ...
                                for (var y = 0; y < lsTarget.languages[x].localizationLanguage.tableList.Count; y++)
                                    if (lsTarget.languages[x].localizationLanguage.tableList[y].table ==
                                        lsTarget.tables[i].localizationTable)
                                    {
                                        lsTarget.languages[x].localizationLanguage.tableList.RemoveAt(y);
                        ...
                        if (lsTarget.tables[i].localizationTable != null)
                            AssetDatabase.DeleteAsset(AssetDatabase.GetAssetPath(lsTarget.tables[i].localizationTable));
                ...
                GUILayout.Space(4);
                GUILayout.BeginHorizontal();
                if (string.IsNullOrEmpty(lsTarget.tables[i].tableID))
                    lsTarget.tables[i].isExpanded = EditorGUILayout.Foldout(lsTarget.tables[i].isExpanded,
                        "Table #" + i, true, foldoutStyle);
                else
                    lsTarget.tables[i].isExpanded = EditorGUILayout.Foldout(lsTarget.tables[i].isExpanded,
                        lsTarget.tables[i].tableID, true, foldoutStyle);
                lsTarget.tables[i].isExpanded = GUILayout.Toggle(lsTarget.tables[i].isExpanded, new GUIContent(""),
                ...
                if (lsTarget.tables[i].isExpanded)
                {
                    ...
                    lsTarget.tables[i].localizationTable = EditorGUILayout.ObjectField("Localization Table",
                        lsTarget.tables[i].localizationTable, typeof(LocalizationTable), true) as LocalizationTable;

                    if (lsTarget.tables[i].localizationTable != null)
                        lsTarget.tables[i].localizationTable.tableID = lsTarget.tables[i].tableID;
                    if (lsTarget.tables[i].localizationTable == null &&
                        GUILayout.Button("+ Create Table Asset", customSkin.button))
                    {
                        var newTable = CreateInstance<LocalizationTable>();

                        // Check for the path
                        var path = AssetDatabase.GetAssetPath(lsTarget);
                        path = path.Replace("/" + lsTarget.name + ".asset", "").Trim();
                        var fullPath = path.Replace("/" + lsTarget.name + ".asset", "").Trim() + "/Tables/";
                    ...
                    else if (lsTarget.tables[i].localizationTable != null &&
                             GUILayout.Button("Show Table Asset", customSkin.button))
                    {
                        if (lsTarget.tables[i].localizationTable.localizationSettings == null)
                            lsTarget.tables[i].localizationTable.localizationSettings = lsTarget;
            ...
            if (lsTarget.tables.Count == 0)
            {
                ...
                {
                    var table = new LocalizationSettings.Table();
                    table.tableID = "Table #" + lsTarget.tables.Count;
                    ...
                    EditorUtility.SetDirty(lsTarget);
                     ...
                     GUILayout.Button("+ New Table", customSkin.button))
            {
                var table = new LocalizationSettings.Table();
                table.tableID = "Table #" + lsTarget.tables.Count;
                lsTarget.tables.Add(table);
            ...
            else if (lsTarget.tables[lsTarget.tables.Count - 1].localizationTable == null)
            {
                EditorGUILayout.HelpBox(
                    "In order to create a new table, you must first create a table asset for " +
                    lsTarget.tables[lsTarget.tables.Count - 1].tableID + ".", MessageType.Warning);
            }
            ...
            enableExperimental.boolValue = HeatUIEditorHandler.DrawToggle(enableExperimental.boolValue, customSkin,
            ...
            EditorGUILayout.HelpBox("Stuck or just getting started? You can check out the documentation page.",
                MessageType.Info);
        ...
        private void RefreshLanguageDropdown()
        {
            for (var i = 0; i < lsTarget.languages.Count; i++)
            {
                ...
                if (string.IsNullOrEmpty(lsTarget.defaultLanguageID) == false &&
                    languageList[i] == lsTarget.defaultLanguageID) defaultLanguageIndex = i;
            }

            if (string.IsNullOrEmpty(lsTarget.defaultLanguageID) && languageList.Count != 0)
                lsTarget.defaultLanguageID = languageList[0];
        }

        private void Docs()
        {
            Application.OpenURL("https://docs.michsky.com/docs/heat-ui/localization");
        }
    }
}
#endif