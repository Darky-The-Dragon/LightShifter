...
namespace Michsky.UI.Heat
{
    public class LocalizationTableWindow : EditorWindow
    {
        ...
        private static readonly float textFieldWidth = 180;
        private static readonly float textFieldHeight = 80;
        private static readonly float itemSpacing = 3;
        ...
        private void OnGUI()
        {
            ...
            if (langLabelStyle == null)
                langLabelStyle = new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter };
            ...
            if (localizationSettings == null || selectedTable == null)
            {
                if (cachedTable != null)
                {
                    selectedTable = cachedTable;
                    localizationSettings = selectedTable.localizationSettings;
                }
                else
                {
                    EditorGUILayout.HelpBox(
                        "There's no selected table. You can reopen the table via Localization Settings or the table file.",
                        MessageType.Info);
                    return;
                }
            }
            ...
            searchString = GUILayout.TextField(searchString, GUI.skin.FindStyle("ToolbarSearchTextField"),
                GUILayout.Width(textFieldWidth + 24));
            if (!string.IsNullOrEmpty(searchString) && GUILayout.Button(new GUIContent("", "Clear search bar"),
            ...
            selectedTable =
                EditorGUILayout.ObjectField(selectedTable, typeof(LocalizationTable), true, GUILayout.Width(200)) as
                    LocalizationTable;
            GUI.enabled = true;
            GUILayout.Space(8);
            ...
            for (var i = 0; i < localizationSettings.languages.Count; i++)
            {
                if (localizationSettings.languages[i].localizationLanguage == null)
                    continue;

                EditorGUILayout.LabelField(new GUIContent(localizationSettings.languages[i].languageName + " (" +
                                                          localizationSettings.languages[i].languageID + ")")
                    , langLabelStyle, GUILayout.Width(textFieldWidth));
                if (i != localizationSettings.languages.Count - 1)
                    EditorGUILayout.LabelField("", GUI.skin.verticalSlider, GUILayout.Width(0));
            }
            ...
            for (var i = 0; i < selectedTable.tableContent.Count; i++)
                // If search field is blank
                if (string.IsNullOrEmpty(searchString))
                {
                    GUILayout.BeginHorizontal();

                    if (GUILayout.Button(new GUIContent("-", "Remove string key"), customSkin.button,
                            GUILayout.Width(20), GUILayout.Height(textFieldHeight)))
                    {
                        Undo.RecordObject(this, "Removed localization string key");

                        for (var x = 0; x < localizationSettings.languages.Count; x++)
                        {
                            localizationSettings.languages[x].localizationLanguage.tableList[tableIndex].tableContent
                                .RemoveAt(i);
                    ...
                    selectedTable.tableContent[i].key = EditorGUILayout.TextArea(selectedTable.tableContent[i].key,
                        GUILayout.Width(textFieldWidth), GUILayout.Height(textFieldHeight));
                    EditorUtility.SetDirty(selectedTable);
                    GUILayout.Space(itemSpacing);

                    for (var x = 0; x < localizationSettings.languages.Count; x++)
                    {
                        if (localizationSettings.languages[x].localizationLanguage == null ||
                            localizationSettings.languages[x].localizationLanguage.tableList[tableIndex].table !=
                            selectedTable)
                            continue;
                        ...
                        if (localizationSettings.enableExperimental)
                        {
                            ...
                            localizationSettings.languages[x].localizationLanguage.tableList[tableIndex].tableContent[i]
                                .value = EditorGUILayout.TextArea(
                                localizationSettings.languages[x].localizationLanguage.tableList[tableIndex]
                                    .tableContent[i].value, EditorStyles.textArea, GUILayout.Width(textFieldWidth),
                                GUILayout.Height(textFieldHeight - 20));
                            localizationSettings.languages[x].localizationLanguage.tableList[tableIndex].tableContent[i]
                                .key = selectedTable.tableContent[i].key;
                            ...
                            localizationSettings.languages[x].localizationLanguage.tableList[tableIndex].tableContent[i]
                                ...
                                EditorGUILayout.ObjectField(
                                    localizationSettings.languages[x].localizationLanguage.tableList[tableIndex]
                                        .tableContent[i].audioValue, typeof(AudioClip), true) as AudioClip;
                            // GUILayout.Label("Sprite", GUILayout.Width(70));
                            localizationSettings.languages[x].localizationLanguage.tableList[tableIndex].tableContent[i]
                                    .spriteValue =
                                EditorGUILayout.ObjectField(
                                    localizationSettings.languages[x].localizationLanguage.tableList[tableIndex]
                                        .tableContent[i].spriteValue, typeof(Sprite), true) as Sprite;
                            GUILayout.EndHorizontal();
                ...
                else if (localizationSettings.languages[0].localizationLanguage.tableList[tableIndex].tableContent[i]
                         .key.ToLower().Contains(searchString.ToLower()))
                {
                    ...
                    GUI.enabled = false;
                    EditorGUILayout.TextArea(selectedTable.tableContent[i].key, GUILayout.Width(textFieldWidth),
                        GUILayout.Height(textFieldHeight));
                    GUI.enabled = true;
                    GUILayout.Space(itemSpacing);
                    ...
                    for (var x = 0; x < localizationSettings.languages.Count; x++)
                    {
                        ...
                        localizationSettings.languages[x].localizationLanguage.tableList[tableIndex].tableContent[i]
                            .value = EditorGUILayout.TextArea(
                            localizationSettings.languages[x].localizationLanguage.tableList[tableIndex].tableContent[i]
                                .value, EditorStyles.textArea, GUILayout.Width(textFieldWidth),
                            GUILayout.Height(textFieldHeight));
                        GUILayout.Space(itemSpacing);
                        localizationSettings.languages[x].localizationLanguage.tableList[tableIndex].tableContent[i]
                            .key = selectedTable.tableContent[i].key;
            ...
            if (GUILayout.Button("+ Add New Table Entry", customSkin.button))
            {
                ...
                var tempContent = new LocalizationTable.TableContent();
                tempContent.key = "Key " + selectedTable.tableContent.Count;
                ...
                for (var i = 0; i < localizationSettings.languages.Count; i++)
                {
                    ...
                    var ltc = new LocalizationLanguage.TableContent();
                ...
                scrollPosition = new Vector2(scrollPosition.x,
                    selectedTable.tableContent.Count * (textFieldHeight + itemSpacing));
            }
        ...
        public static void ShowWindow(LocalizationSettings settings, LocalizationTable table, int index)
        {
            ...
            var newTitle = new GUIContent("Localization Table (" + table.tableID + ")", icon);
