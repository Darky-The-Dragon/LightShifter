...
namespace Michsky.UI.Heat
{
    ...
    public class LocalizationLanguageEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Import Table", customSkin.button))
            {
                Import();
                return;
            }

            if (GUILayout.Button("Export Table(s)", customSkin.button))
            {
                Export();
                return;
            }
        ...
        private void Import()
        {
            var path = EditorUtility.OpenFilePanel("Select a file to import", "", "");

            if (path.Length != 0)
            {
                string tempKey = null;
                var checkForValue = false;
                var processNewEntry = false;

                LocalizationTable targetTable = null;
                var keysToBeAdded = new List<LocalizationLanguage.TableContent>();

                foreach (var option in File.ReadLines(path))
                {
                    if (option.Contains("[LanguageID] "))
                    {
                        var tempLangID = option.Replace("[LanguageID] ", "");
                        checkForValue = false;
                    ...
                    {
                        var tempTableID = option.Replace("[TableID] ", "");
                        checkForValue = false;

                        for (var i = 0; i < lTarget.tableList.Count; i++)
                    ...
                    else if (option.Contains("[Value] "))
                    {
                        ...
                        for (var i = 0; i < tempTable.tableContent.Count; i++)
                            if (tempTable.tableContent[i].key == tempKey)
                            {
                                processNewEntry = false;
                                var tempValue = option.Replace("[Value] ", "");
                                tempTable.tableContent[i].value = tempValue;
                            }

                        if (processNewEntry)
                        {
                            var tempEntry = new LocalizationLanguage.TableContent();
                    ...
                    else if (checkForValue && !option.Contains("[Value] ") && !string.IsNullOrEmpty(option))
                    {
                        if (tempTable == null)
                        {
                            Debug.LogError("Can't find the given table ID.");
                        ...
                        if (processNewEntry)
                            keysToBeAdded[keysToBeAdded.Count - 1].value =
                                keysToBeAdded[keysToBeAdded.Count - 1].value + "\n" + option;
                        else
                            for (var i = 0; i < tempTable.tableContent.Count; i++)
                                if (tempTable.tableContent[i].key == tempKey)
                ...
                {
                    for (var i = 0; i < keysToBeAdded.Count; i++)
                    {
                        var newTableEntry = new LocalizationTable.TableContent();
                        newTableEntry.key = keysToBeAdded[i].key;
                        targetTable.tableContent.Add(newTableEntry);

                        for (var x = 0; x < lTarget.localizationSettings.languages.Count; x++)
                        {
                            foreach (var list in lTarget.localizationSettings.languages[x].localizationLanguage
                                         .tableList)
                    ...
                    Debug.Log(keysToBeAdded.Count + " new localization entries has been added.");
                    Debug.Log(tempTable.table.tableID + " (table) has been successfully imported to " +
                              lTarget.languageID);
        ...
        private void Export()
        {
            for (var i = 0; i < lTarget.tableList.Count; i++)
            {
                var path = EditorUtility.SaveFilePanel("Export: " + lTarget.tableList[i].table.tableID, "",
                    "Exported_" + lTarget.tableList[i].table.tableID + "(" + lTarget.languageID + ")", "txt");
                ...
                if (path.Length != 0)
                {
                    ...
                    for (var x = 0; x < lTarget.tableList[i].table.tableContent.Count; x++)
                    {
